...
void PrintStep(const std::string &s, int32_t count) {
  printf("%-14s%14" PRId64 "\n", (s + ":").c_str(), count);
}

void PrintStep(int step, double seconds, int32_t count = -1) {
  if (count != -1)
...
void PrintStep(const std::string &s, double seconds, int32_t count = -1) {
  if (count != -1)
